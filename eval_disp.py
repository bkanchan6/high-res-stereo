...
parser.add_argument("--method", type=str, default="HSM", help="resolution")
parser.add_argument("--indir", type=str, default="/ssd/carla-highres/carlaF/", help="resolution")
parser.add_argument("--gtdir", type=str, default="/ssd/carla-highres/testF/", help="resolution")
...
for level in levels:
    ...
    for i, imgname in enumerate(imgnames):
        ...
        if imgname.split("/")[-1] in blist.keys():
            for b in blist[imgname.split("/")[-1]]:
                mask[int(b[0][1]) : int(b[1][1]), int(b[0][0]) : int(b[1][0])] = 0
    ...
    times[-1] = times.mean()
    t.add_rows(
        list(zip([i.split("/")[-1] for i in imgnames], avgerrs, rmss, bad5s, bad10s, bad20s, times))[-1:], header=False
    )
