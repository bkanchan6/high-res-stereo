...
scale_factor = args.maxdisp / 384.0  # controls training resolution
...
all_left_img, all_right_img, all_left_disp, all_right_disp = ls.dataloader(
    "%s/mb-ex-training/trainingF" % args.database
)  # mb-ex
loader_mb = DA.myImageFloder(
    all_left_img,
    all_right_img,
    all_left_disp,
    right_disparity=all_right_disp,
...
def train(imgL, imgR, disp_L):
    ...
    loss = (
        (64.0 / 85) * F.smooth_l1_loss(stacked[0][mask], disp_true[mask], size_average=True)
        + (16.0 / 85) * F.smooth_l1_loss(stacked[1][mask], disp_true[mask], size_average=True)
        + (4.0 / 85) * F.smooth_l1_loss(stacked[2][mask], disp_true[mask], size_average=True)
        + (1.0 / 85) * F.smooth_l1_loss(stacked[3][mask], disp_true[mask], size_average=True)
    )
    loss.backward()
    optimizer.step()
    vis = {}
