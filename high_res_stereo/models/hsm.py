    ...
    def forward(self, left, right):
        ...
        if self.level > 2:
            ...
            if self.level > 1:
                cost3 = F.upsample(
                    (cost4).unsqueeze(1),
                    [self.disp_reg8.disp.shape[1], left.size()[2], left.size()[3]],
                    mode="trilinear",
                ).squeeze(1)
            else:
        ...
        if self.training:
            cost6 = F.upsample(
                (cost6).unsqueeze(1), [self.disp_reg8.disp.shape[1], left.size()[2], left.size()[3]], mode="trilinear"
            ).squeeze(1)
            cost5 = F.upsample(
                (cost5).unsqueeze(1), [self.disp_reg8.disp.shape[1], left.size()[2], left.size()[3]], mode="trilinear"
            ).squeeze(1)
