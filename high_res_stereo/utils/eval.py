...
def eval_carla(result_dir, method_name, img_name, th):
    ...
    command = "code/evaldisp %s/%s%s.pfm /data/gengshay/carla-new-eval/%s.pfm %f 768 0" % (
        ...
        method_name,
        image_suffix,
        image_suffix.replace("cam0", "depth0"),
        th,
    )
    command = command.replace("XX_XX", img_name)
    lines = [i for i in run_command(command.split())]
        ...
        if os.path.isdir(filename):
            call(
                [
                    "cp",
                    "-rf",
                    "submission_results/Middlebury2014_iResNet_ROB/val/%s" % imname,
                    "/home/gengshay/code/MiddEval3/val%s/" % (rdir),
                ]
            )
...
def run_sgm(eval_dir, subset, rdir, res, img_name):
    ...
    # prepare images
    command = (
        "SGM/app %s/OO_OO%s/XX_XX/im0tmp.png %s/OO_OO%s/XX_XX/im1tmp.png -dst_path=%s/OO_OO%s/XX_XX/disp0tmp.png -max_disparity=%d -no-downscale"
        % (eval_dir, rdir, eval_dir, rdir, eval_dir, rdir, max_disp)
    )
    ...
    # save pfm
    disp = np.asarray(PIL.Image.open("%s/%s%s/%s/disp0tmp.png" % (eval_dir, subset, rdir, img_name)))[
        max_disp * 2 : -max_disp * 2, max_disp * 2 : -max_disp * 2
    ]
    ...
    with open("%s/%s%s/%s/disp0SGM.pfm" % (eval_dir, subset, rdir, img_name), "w") as f:
        save_pfm(f, disp.astype(np.float32)[::-1], scale=1.0 / max_disp)
...
def run_mccnn(eval_dir, subset, rdir, res, img_name):
    ...
    os.chdir("mc-cnn")
    command = (
        "./main.lua mb slow -a predict -net_fname net/net_mb_slow_-a_train_all.t7 -left %s/OO_OO%s/XX_XX/im0tmp.png -right %s/OO_OO%s/XX_XX/im1tmp.png -disp_max %d"
        % (eval_dir, rdir, eval_dir, rdir, max_disp)
    )
    command = command.replace("XX_XX", img_name)
    command = command.replace("OO_OO", subset)
    begt = time.time()
